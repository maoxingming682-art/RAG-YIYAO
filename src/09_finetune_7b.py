"""
09_qlora_finetune.py (7B升级版)
LoRA微调 Qwen2.5-7B-Instruct - 纯PyTorch循环
优化：gradient_checkpointing + batch_size=1 + max_length=384
适配16GB显存
"""
import os
import sys
...
LORA_DROPOUT = 0.05
EPOCHS = 3
BATCH_SIZE = 1         # 7B用1（省显存）
GRAD_ACCUM = 8         # 等效batch_size=8
LEARNING_RATE = 1e-4   # 7B用稍小学习率
MAX_LENGTH = 384       # 缩短序列省显存
OUTPUT_DIR = os.path.join(BASE_DIR, "lora_output_7b")
os.environ["HF_ENDPOINT"] = "https://hf-mirror.com"


def finetune():
    print("=" * 60)
    print("第3步：LoRA 微调 Qwen2.5-7B-Instruct")
    print("=" * 60)
    print(f"模型: {MODEL_NAME}")
    print(f"LoRA: r={LORA_R}, alpha={LORA_ALPHA}")
    print(f"训练: epochs={EPOCHS}, batch={BATCH_SIZE}, accum={GRAD_ACCUM}")
    print(f"序列长度: {MAX_LENGTH}")
    print(f"优化: gradient_checkpointing + fp16", flush=True)
    ...
    # 2. 加载7B模型 fp16
    print("加载7B模型（fp16，约14GB下载）...", flush=True)
    ...
    # 开启gradient checkpointing省显存（用计算换显存，省~30%）
    model.gradient_checkpointing_enable()
    model.enable_input_require_grads()

    gpu_mem = torch.cuda.memory_allocated() / 1024**3
    print(f"7B模型加载完成, GPU显存: {gpu_mem:.1f} GB", flush=True)
    ...
    for item in raw_data:
        messages = [
            {"role": "system", "content": "你是专业药学咨询助手。根据问题类型用不同风格回答：用法用量简洁直接，不良反应列举提醒，禁忌人群重点警告，非用药问题正常对话。"},
            ...
            if accum_count >= GRAD_ACCUM:
                ...
                if step % 5 == 0:
                    ...
                    print(f"  epoch {epoch+1}/{EPOCHS} step {step}/{total_steps} "
                          ...
                          f"remaining={remaining:.0f}s GPU={peak:.1f}GB",
                          flush=True)

        print(f"Epoch {epoch+1} 完成, 平均loss={epoch_loss/epoch_count:.4f}", flush=True)
